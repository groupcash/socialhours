...
class AuthorizeCreditorSpec extends SocialHoursSpecification {
    ...
    function invalidToken() {
        $this->when->tryTo(new AuthorizeCreditor(new Token('wrong token'), 'creditor@foo'));
        $this->then->shouldFail('Invalid token.');
    }

    function notAdministrator() {
        $this->given(new TokenGenerated(Time::now(), new Token('that token'), 'notAdmin@foo'));
        $this->when->tryTo(new AuthorizeCreditor(new Token('that token'), 'creditor@foo'));
    ...
    function accountNotExisting() {
        ...
        $this->given(new TokenGenerated(Time::now(), new Token('that token'), 'foo@bar'));
        $this->when->tryTo(new AuthorizeCreditor(new Token('that token'), 'creditor@foo'));
    ...
    function success() {
        ...
        $this->when(new AuthorizeCreditor(new Token('that token'), 'creditor@foo'));
